...
turtles-own [
  ...
  Mdens_Ec_baby                               ; Starting density of carbon-rich reserve for juvenille  [mol C / mol C]
  Mdens_En_baby                               ; Starting density of nitrogen-rich reserve for juvenille  [mol C / mol C]
...
to setup
  ...
  sr:setup
  sr:run "source('C:/Users/jj0895/Documents/Projects/UNT_projects/woodstoich/code/DEBmodel_ODEsolver_for_NetLogo.R')"
...
to go
  ...
  have-baby                              ; Allow juvenilles to appear
...
to color-patches
  ...
  ask resource-patches [
    set pcolor palette:scale-gradient [[84 83 60] [109 114 64] [123 132 69] [112 148 79] [94 175 93]] resource-CN-ratio min-cn-color-value max-cn-color-value
...
to initialize-DEB-parameters
  ...
  set M_H_p 0.25 ; Lower than DEB Add-My-Pet parameter
  set M_V_baby 0.00424512  ; Lower than DEB Add-My-Pet parameter
  set M_R_bufferbaby 0.2591115 ; Calculation done for a pup to meet size, maturity, and reserve densities at birth
  set avg_litter_size 5
  set M_R_bufferlitter (M_R_bufferbaby * avg_litter_size)
  let scaled_mdens_Ec_baby 0.9
  let scaled_mdens_En_baby 0.8569745
  set mdens_Ec_baby (scaled_mdens_Ec_baby * mdens_Ec_max)
  set mdens_En_baby (scaled_mdens_En_baby * mdens_En_max)
...
to initialize-DEB-variables
  ...
  ifelse is-adult? = 0 [
    ...
    set M_V (M_V_baby * (1 + (random-float 0.2 - 0.1)))            ; Size (biomass)
    set M_H M_H_baby + (random-float (M_H_p * 0.99 - M_H_baby))    ; Maturity
    set M_R 0                                                      ; Reproduction buffer
  ][
    set M_V (0.2 * (1 - random-float 0.1))                         ; Size (biomass)
  ...
  set consumer-total-Nmol (((q-V_N / q-V_C) * M_V) + ((q-En_N / q-En_C) * M_En) + ((q-R_N / q-R_C) * M_R))  ; Total composition of consumer that is mol N
...
to move
  ...
  ask turtles [
    ...
    if scaled_Mdens_En <= N-reserve-threshold [
      ;;===== Seek high nitrogen food when body nitrogen (C:N) is above the threshold
      let best-patch max-one-of patches in-radius 10 [resource-quantity-Nmol] ;; Look for best nitrogen patch nearby

      ;;===== Check if best patch exists (avoid errors)
      if best-patch != nobody [
        ;;======= Go to best patch nearby
        face best-patch                  ; Face the patch with the highest nitrogen
        forward distance-consumer-walks  ; Move to the target patch

        ;;======= Check if the target patch is occupied by more than one turtle
        if count turtles-here > 1 [
          let empty-patches patches in-radius 1 with [not any? turtles-here]  ; Look for nearby unoccupied patches

          ;;========= If there are unoccupied patches, move to one
          if any? empty-patches [
            let alternative-patch max-one-of empty-patches [resource-quantity-Nmol]
            if alternative-patch != nobody [
              face alternative-patch
              move-to alternative-patch
            ]
          ]
        ]
      ]
  ...
  ;;= If turtles do not have a low nitrogen-rich reserve, they will move according to either brownian-walk or a correlated-random-walk, as determined by user
  ask turtles [
    if scaled_Mdens_En > N-reserve-threshold [
      if movement-type = "brownian-walk" [
        ;;=== Attempt a random move, but if only if there is not another turtle on there
        let target-patch patch-ahead distance-consumer-walks
        if target-patch != nobody [
          ifelse not any? turtles-on target-patch [
            forward distance-consumer-walks
          ] [
            ;;===== If target patch is occupied, look for nearby unoccupied patches
            let empty-patches patches in-radius 1 with [not any? turtles-here]
            if any? empty-patches [
              let alternative-patch one-of empty-patches
              forward distance-consumer-walks
            ]
          ]
        ]
...
to calculate-DEB-ODE
 ;;= Calculate change in consumer state variables this tick, performed in R using the SimpleR NetLogo Extension
  ask turtles [
    ...
    set netlogo_input lput Food netlogo_input
...
to update-turtle-variables
  ...
  ask turtles[
    ...
    set consumer-total-Nmol (((q-V_N / q-V_C) * M_V) + ((q-En_N / q-En_C) * M_En) + ((q-R_N / q-R_C) * M_R))
  ...
  ask turtles [
    if M_R >= M_R_bufferlitter [
      ...
      hatch avg_litter_size [
        ...
        set V (M_V / M_V_SqBrack)
        set L (V ^ (1 / 3))
        set Mdens_Ec mdens_Ec_max
        set Mdens_En mdens_En_max
        set scaled_Mdens_Ec (Mdens_Ec / mdens_Ec_max)
        set scaled_Mdens_En (Mdens_En / mdens_En_max)
        set M_Ec (Mdens_Ec * M_V)
        set M_En (Mdens_En * M_V)
...
to have-die?
  ...
  ask turtles [
   ;;=== Starvation scenarios
    if (scaled_Mdens_Ec <= 0.05) and (scaled_Mdens_En <= 0.05) [
...
to patch-N-added-by-turtles
  ...
  ask turtles [
    ...
    ask patch-here [
      set turtle-alterations-patch-pooled-N [turtle-input-N-this-tick] of myself
    ]
  ]
end
...
to update-patches
  ...
  ask resource-patches[
    ...
    let resource-CN-ratio-test (max-total-Cmol / max-total-Nmol)
...
to plot-consumer-N-output
  set-current-plot "Total Consumer N Put on Patches"
end
@#$#@#$#@
GRAPHICS-WINDOW
5
...
1454
88
% of resource patches in high-quality clusters
...
"" ""
PENS
"Total" 1.0 0 -8630108 true "" "plot total-consumers"
"Adults" 1.0 0 -13345367 true "" "plot total-adult-consumers"
"Juveniles" 1.0 0 -4699768 true "" "plot total-juvenille-consumers"
...
1454
427
Total Consumer N Put on Patches
NIL
NIL
0.0
10.0
...
"" ""
PENS
"default" 1.0 0 -8431303 true "" "plot sum [turtle-alterations-patch-pooled-N] of patches"
...
@#$#@#$#@
<experiments>
  <experiment name="ScalingImbalance_SimulationExperiment" repetitions="8" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <steppedValueSet variable="N-reserve-threshold" first="0" step="0.2" last="0.6"/>
    ...
    <steppedValueSet variable="movement-somatic-maintenance-cost-multiplier" first="0" step="0.5" last="1.5"/>
    ...
    <enumeratedValueSet variable="num-resource-clusters">
      <value value="10"/>
    </enumeratedValueSet>
  ...
  <experiment name="ScalingImbalance_SimulationExperiment_longerrunforcycling" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <metric>count turtles with [is-adult? = 1]</metric>
    <metric>mean [M_V] of turtles</metric>
    <metric>variance [M_V] of turtles</metric>
    <metric>mean [M_Ec] of turtles</metric>
    <metric>variance [M_Ec] of turtles</metric>
    <metric>mean [M_En] of turtles</metric>
    <metric>variance [M_En] of turtles</metric>
    ...
    <enumeratedValueSet variable="initial-mean-carbon-reserve-filled-fraction">
      <value value="0.7"/>
    </enumeratedValueSet>
    <steppedValueSet variable="N-reserve-threshold" first="0" step="0.6" last="0.6"/>
    <enumeratedValueSet variable="patch-soil-N-multiplier">
      <value value="0.5"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="resource-patch-aggregation">
    ...
    <enumeratedValueSet variable="initial-mean-nitrogen-reserve-filled-fraction">
      <value value="0.7"/>
    </enumeratedValueSet>
  </experiment>
  <experiment name="woodstoich_runforJim" repetitions="8" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="1825"/>
    <exitCondition>not any? turtles</exitCondition>
  ...
  <experiment name="woodstoich_N-reserve_long-runs" repetitions="8" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    ...
    <enumeratedValueSet variable="movement-somatic-maintenance-cost-multiplier">
      <value value="0"/>
      <value value="1"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="percent-world-no-resources">
